...
Your polynomial extrapolations should fail spectacularly. In today's terms, we'd call Gauss's approach "physics-informed".

1. **Fitting a Conic Section**: Gauss knew from Kepler's First Law that planetary orbits are **ellipses**. The general equation for a conic section (ellipse, parabola, or hyperbola) is:
...
2. **The Time Problem**: Even with an ellipse fit, Gauss faced a harder problem: predicting *where* Ceres would be at a *future time*. The relationship between position and time on an ellipse is governed by [**Kepler's Equation**](https://en.wikipedia.org/wiki/Kepler%27s_equation):

   $$M = \kappa - e \sin(\kappa)$$

   where $$M = \frac{2\pi}{T}(t - t_0)$$ is the "mean anomaly" (proportional to time), $$\kappa$$ is the "eccentric anomaly" (an auxiliary angle), $$e$$ is the orbital eccentricity, and $$T$$ is the orbital period.

   (a) Given $$M$$, we need to solve for $$\kappa$$. Is Kepler's equation **linear or nonlinear** in $$\kappa$$? Can you solve it analytically?
...
**Final Reflection**: In 2-3 sentences, explain what Gauss gained by assuming an elliptical orbit that polynomial fitting couldn't provide.
