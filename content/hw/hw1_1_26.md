...
In 1676, Robert Hooke discovered the law of elasticity, which states that the extension of a spring is proportional to the force applied to it. In class, we took photos of the extension of the spring for different masses. You are provided with the images and the masses, in grams, used in the experiment. Your task is to use linear regression to find the parameters of Hooke's law using both the normal equation and stochastic gradient descent. The true test of your model will be to submit predictions to a Kaggle competition in which you have to estimate the mass hanging in new images.

(a) Create an account and participate in the [Kaggle competition](https://www.kaggle.com/competitions/hookes-law-with-a-computer/) where you'll get access to the training and test images. Extract the data from the images and create a dataset $$\{ (x^{(i)}, y^{(i)}) \}_{i=1}^{n}$$ where $$x^{(i)}$$ is the extension of the spring in image $$i$$ (in pixels) and $$y^{(i)}$$ is the mass hanging from the spring, in grams, which is what the data gives you. Hooke's law relates extension to force rather than to mass, so the slope you fit absorbs the constant that converts grams to Newtons. You can use any method you like to extract the data from the images (e.g, image processing, etc.). Provide a brief description of your method.
